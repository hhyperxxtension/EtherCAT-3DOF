...
Полная схема итогового алгоритма с непрерывным адаптивным наблюдателем
нагрузки приведена на рис.~\ref{fig:block}: измеренный момент
де-инерционируется и де-фрикционируется по динамической модели
(инерция, Кориолис, трение), остаток даёт текущую нагрузку (включая
неизвестный груз), из неё формируется поправка $\delta$, добавляемая к
траектории в реальном времени.

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{fig_blockdiagram.png}
\caption{Итоговый алгоритм: верх~--- конвейер реального времени (4\,мс); центр~---
наблюдатель нагрузки (динамическая модель $\to$ остаток $\to$ груз $c_p$ $\to$
фильтр $\to$ деформация $\delta$); низ~--- офлайн-калибровки, питающие модели.}
\label{fig:block}
\end{figure}
